...
`;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      ...
      if (lastResponse && lastResponseTime && Date.now() - lastResponseTime < 5e3) {
        const { options: options2, summary } = splitOptionsAndSummary(lastResponse);
        displayResponses(options2, summary);
      } else {
  ...
  function splitOptionsAndSummary(markdown) {
    const parts = markdown.split(/\n(?=\*\*Option \d+)/g).map((s) => s.trim()).filter(Boolean);
    let options2 = parts.length > 1 && !parts[0].startsWith("**Option") ? parts.slice(1) : parts;
    let summary = void 0;
    if (options2.length > 0) {
      const last = options2[options2.length - 1];
      ...
      if (summaryIndex !== -1) {
        ...
        options2[options2.length - 1] = lines.slice(0, summaryIndex).join("\n").trim();
      }
    }
    const optionObjs = options2.map((opt) => {
  ...
  function displayResponses(responses, summary) {
    const container = document.getElementById("responses");
    if (!container) return;
    container.innerHTML = "";
    responses.forEach((response, index) => {
      ...
      try {
        ...
        const html2 = marked.parse(strippedContent);
        if (typeof html2 === "string") {
          content.innerHTML = html2;
        } else if (html2 instanceof Promise) {
          html2.then((res) => content.innerHTML = res);
        }
      } catch (error) {
  ...
  function stripBlockquotes(markdown) {
